...
class GameRenderer:
    ...
    def draw_game(self, screen: Any, snapshot: Snapshot, fonts: tuple[Any, Any, Any]) -> None:
        ...
        if snapshot.message:
            center_text(screen, fonts[0], snapshot.message, (255, 230, 0), screen.get_height() - 25, self._pygame)
    ...
    def draw_end_screen(
        ...
        """Draw GAME OVER or VICTORY neon dialog card matching reference image."""
        ...
        # Red for GAME OVER, Green for VICTORY
        border_color = (255, 50, 70) if is_over else (50, 235, 100)
        draw_neon_menu_card_frame(screen, card_rect, border_color, self._pygame, pacman_icon=self._pacman_icon)
        ...
        center_text(screen, title_font, title_text, title_color, card_rect.top + int(card_h * 0.16), self._pygame)

        # "SCORE" Header
        cyan_font = self._get_font(int(card_h * 0.045))
        center_text(screen, cyan_font, "SCORE", (0, 210, 255), card_rect.top + int(card_h * 0.36), self._pygame)

        # Score Value (e.g. 002340)
        score_font = self._get_font(int(card_h * 0.07))
        score_str = f"{snapshot.score:06d}"
        center_text(screen, score_font, score_str, (245, 245, 245), card_rect.top + int(card_h * 0.47), self._pygame)

        # Name Entry Prompt
        prompt_font = self._get_font(int(card_h * 0.05))
        prompt_text = "Saved. Press Enter" if saved else f"Name (max 10): {name}_"
        center_text(screen, prompt_font, prompt_text, (230, 230, 240), card_rect.top + int(card_h * 0.72), self._pygame)

    def _draw_hud(self, screen: Any, snapshot: Snapshot, fonts: tuple[Any, Any, Any]) -> None:
        """Original restored HUD."""
        hud = (
    ...
    def _draw_maze(self, screen: Any, maze: Any, left: int, top: int, cell: int) -> None:
        for y, row in enumerate(maze.cells):
            for x, walls in enumerate(row):
                ...
                if walls & 1:
                    draw_neon_line(screen, (px, py), (px + cell, py), wall_color, self._pygame)
                if walls & 2:
                    draw_neon_line(screen, (px + cell, py), (px + cell, py + cell), wall_color, self._pygame)
                if walls & 4:
                    draw_neon_line(screen, (px, py + cell), (px + cell, py + cell), wall_color, self._pygame)
                if walls & 8:
                    draw_neon_line(screen, (px, py), (px, py + cell), wall_color, self._pygame)
    ...
    def _draw_items(self, screen: Any, snapshot: Snapshot, left: int, top: int, cell: int) -> None:
        for position, item in snapshot.items:
            ...
            draw_neon_circle(screen, center, radius, (255, 226, 140), self._pygame)
    ...
    def _draw_player(self, screen: Any, snapshot: Snapshot, left: int, top: int, cell: int) -> None:
        ...
        if self._atlas is not None:
            ...
            draw_neon_circle(screen, player_center, max(4, cell // 2 - 2), (255, 222, 0), self._pygame)
        ...
        for index, g_ent in enumerate(snapshot.ghost_visual_positions):
            ...
            if self._atlas is not None:
                ...
                if g_ent.mode is not GhostMode.RESPAWNING:
                    draw_neon_circle(screen, center, max(4, cell // 2 - 2), color, self._pygame)
