...
| **R-04** | Rendering becoming coupled with gameplay logic.               |   High   |    High    | Keep rendering completely independent through immutable `snapshots` while the engine operates on logical grid positions.                                                     |
...
| **R-06** | Visual and logical collision mismatch.                        |   High   |    High    | Use explicit `Hitbox` dimensions and interpolated entity positions to detect actual axis-aligned rectangular overlap instead of relying on logical cell equality.            |
